...
const Test = ({ children, name = 'test', priority, append, prepend }: TestProps) => {
  useSetElement({
    append,
    prepend,
    priority,
  ...
  return (
    <SetElement name={name} priority={priority} append={append} prepend={prepend}>
...
describe('GetElement / SetElement', () => {
  ...
  it('renders new highest when it mounts', () => {
    const App = ({ showOne, showTwo }: { showOne?: boolean, showTwo?: boolean}) => (
      <OnusElementsProvider>
        <div data-testid='get-element'>
          <GetElement name='test' />
        </div>
